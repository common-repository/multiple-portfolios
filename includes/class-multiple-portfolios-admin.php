 ...
 * @author  Quema Labs
 */
class Multiple_Portfolios_Admin {
	...
	public function init() {
		...
		if ( $multiple_portfolios ) {
			...
			foreach ( $multi_portfolios_names as $multi_portfolios_name ) {
				...
				$this->post_type = $multi_portfolios_name;
				$this->taxonomies = $taxonomy_objects;
				...
				if ( $supported !== true ) {
					...
					if ( is_array( $supported ) ) {
						array_push( $supported, $this->post_type['slug'] );
					} else {
						$supported = array( $this->post_type['slug'] );
					}
				...
				add_filter( 'manage_edit-' . $this->post_type['slug'] . '_columns', array( $this, 'add_thumbnail_column'), 10, 1 );
	...
	public function add_thumbnail_column( $columns ) {
		$column_thumbnail = array( 'thumbnail' => __( 'Thumbnail', 'multiple-portfolios' ) );
	...
	public function add_taxonomy_filters() {
		...
		if ( $this->post_type['slug'] !== $typenow ) {
			return;
		}
	...
	public function add_glance_counts() {
		$glancer = new Multiple_Portfolios_Gamajo_Dashboard_Glancer;
		$glancer->add( $this->post_type['slug'], array( 'publish', 'pending' ) );
	}
	...
	public function add_rightnow_counts() {
		$glancer = new Multiple_Portfolios_Gamajo_Dashboard_RightNow;
		$glancer->add( $this->post_type['slug'], array( 'publish', 'pending' ) );
	}
